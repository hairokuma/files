...
export default class FilesView {
  ...
  constructor(uri, files) {
    ...
    this.uri = uri
    this.media = null;
    this.play = atom.config.get('files.Autoplay.all') ? atom.config.get('files.Autoplay.all') : false;
    ...
    this.setContent(uri);
    const title = () => {
      this.tabTitle = $('div.title[data-name="' + this.getTitle() + '"]');
    }
  ...
  next() {
    ...
    this.setContent(this.files[this.current])
    // atom.workspace.open(this.files[this.current]);
  }
  togglePlay() {
    this.play = !this.play;
  ...
  setTabTitle() {
    ...
    if (this.tabTitle) {
      this.tabTitle.html(this.getTitle());
      this.tabTitle.attr('data-name', this.getTitle());
      this.tabTitle.attr('data-path', this.uri);
    }
  }
  getTitle() {
